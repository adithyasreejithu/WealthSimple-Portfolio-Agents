...
export default async function HoldingDetailPage({
  ...
  const targetWeight = fields.target_weight_percent ?? null;
  ...
  return (
    <div className="space-y-4">
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink render={<Link href={isEtf ? "/etfs" : "/stocks"} />}>
              {isEtf ? "ETFs" : "Stocks"}
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>{holding.ticker_symbol}</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {classification ? <Badge variant="secondary">{classification.primary_group}</Badge> : null}
        {classification?.confidence ? (
          <Badge variant="outline" className="capitalize">{classification.confidence} confidence</Badge>
        ) : null}
        {holding.has_provisional_activity ? (
          <Badge variant="outline" className="text-amber-600">Provisional activity</Badge>
        ) : null}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        ...
        <KpiCard
          label="Last price"
          value={fmtCcy(holding.last_price, holding.currency)}
          sub={fmtDate(holding.last_price_date)}
        ...
        <KpiCard
          label="Unrealized"
          value={<span className={signClass(holding.unrealized_mkt)}>{fmtCcy(holding.unrealized_mkt, holding.currency)}</span>}
          sub={pnlPct === null ? undefined : fmtSignedPct(pnlPct)}
          subClassName={signClass(holding.unrealized_mkt)}
        />
      ...
      <Card>
        ...
        <CardContent>
          <PriceExplorer key={holding.ticker_symbol} initialSymbol={holding.ticker_symbol} />
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            {classification ? (
              <>
                {classification.secondary_tags.length ? (
                  <div className="flex flex-wrap gap-1">
                    {classification.secondary_tags.map((tag) => (
                ...
                <FactRow
                  label="Target vs actual weight"
                  value={targetWeight !== null ? `${fmtNumber(actualWeightPct)}% vs ${fmtNumber(Number(targetWeight))}% target` : `${fmtNumber(actualWeightPct)}% (no target)`}
                />
                {classification.review_needed ? (
                  <Badge variant="destructive" className="font-normal">Needs review</Badge>
                ) : null}
              </>
            ) : (
              <p className="text-muted-foreground">No classification on record for this holding.</p>
            )}
          </CardContent>
        </Card>
      ...
      {holding.data_quality_flags.length ? (
        <p className="text-muted-foreground text-xs">
          Data quality flags: {holding.data_quality_flags.join(", ")}
        </p>
      ) : null}
    </div>
  );
