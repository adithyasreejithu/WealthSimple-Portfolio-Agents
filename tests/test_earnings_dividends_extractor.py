...
class FakeTicker:
    def __init__(self, earnings_dates=None, dividends=None, raises=False):
...
class EarningsDividendsExtractorTest(unittest.TestCase):
    ...
    def test_parse_args_rejects_skipping_both(self):
        with patch("sys.stderr", new_callable=StringIO), self.assertRaises(SystemExit):
            ede.parse_args(["--tickers", "AAPL", "--skip-earnings", "--skip-dividends"])
